'''
This program checks if a maze is solvable by using a Stack object
'''
from Stack import Stack     # call Stack class from Stack.py

def main():
    myStack = Stack()
    myMaze = Maze("unsolvable_maze.txt")
    square = myMaze.get_start_square()
    myStack.push(square)
    success = False
    while not myStack.is_empty():
        #print(square.get_location())
        square = myStack.pop()    
        if myMaze.is_finish_square(square):
            success = True
            break
        else:
    ...
    if success:
        print("Success! The maze is solvable")
    else:
        print("Fail. The maze is unsolvable")
